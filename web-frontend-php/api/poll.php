<?php
  ...
  function getSquadInfo($name) {
    ...
    $q=$db->prepare("
      ...
      WHERE tblSquads.name = ? AND tblPlayerInfo.upd BETWEEN datetime('now', '-5 minutes') AND datetime('now', '+1 year')
    ;");
    $q->execute(array($name));
  ...
  function getMyInfo($name) {
    $db = dbConnect();
    $q=$db->prepare("
      SELECT x, y, map, phase, mode, teamloc, teamdata, flightpath, kills, headshots, damagedealt, maxkilldist, alives, inv, white, blue, red
      FROM tblPlayerInfo
      WHERE name = ?
    ;");
    $q->execute(array($name));
  ...
  function getDeadInfo($name) {
    $db = dbConnect();
    $q=$db->prepare("
      SELECT alives, white, blue, red, upd, teamloc
      FROM tblSquads
      INNER JOIN tblPlayerInfo ON (tblSquads.teammate = tblPlayerInfo.name)
  ...
  function buildResponse($name) {
    ...
    if ($myInfo['phase']=='dead') {
      ...
      if (count($deadInfo)>0) {
        $myInfo['teamloc'] = $deadInfo[0]['teamloc'];
